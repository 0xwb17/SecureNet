...
1.  **Interception:** Edge router catches HTTP traffic and redirects to SecureNet.
2.  **Validation:** SecureNet validates credentials against the MariaDB database.
3.  **Enforcement:** SecureNet triggers a script/API to open the firewall for that specific IP.
4.  **Accounting:** SecureNet logs the session and monitors device presence via ARP.
...
1.  Flash OpenWrt on your edge router.
2.  Install openNDS (Network Detection Service).
3.  Configure openNDS PreAuth to forward all unauthorized traffic to your SecureNet server IP (e.g., http://192.168.1.10:80).
4.  In app.py, replace the simulated firewall rules with actual API calls to openNDS to dynamically authorize MAC addresses.
5.  (Optional) Setup Nginx as a reverse proxy on the server to handle SSL/HTTPS, satisfying modern OS HSTS requirements.
